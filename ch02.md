...
Ejercicio. Usando el objeto `persona` que definir arriba, ¿cómo podríamos acceder a la propiedad `residentes` usando las dos formas?
...
Si vuelves a imprimir después `window`, verás que `variable` está en la lista. Será difícil de encontrar
